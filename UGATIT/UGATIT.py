...
class UGATIT(object) :
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            trainA = tf.data.Dataset.from_tensor_slices(self.trainA_dataset)#该函数的作用是接收tensor，对tensor的第一维度进行切分，并返回一个表示该tensor的切片数据集
            trainB = tf.data.Dataset.from_tensor_slices(self.trainB_dataset)#trainB_dataset是训练集图片路径列表
            ...
            #shuffle_and_repeat洗牌和重复数据集，每次重复都重新洗牌。
